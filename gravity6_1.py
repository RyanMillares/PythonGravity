...
def main():
    ...
    prev_y = -1
    prev_x = -1
    win = GraphWin('Space v6.1', x_dims, y_dims) # 4/13
    ...
    while(True):
        ...
        for circle, choose_color, num, mass in circles:
            ...
            velocities[num].x += ((x_dist / (curr_dis ** 2)) * gravity_constant * (mass * center_gravity) / ((mass/force_multiplier)**2)) / sim_step_speed
            velocities[num].y += ((y_dist / (curr_dis ** 2)) * gravity_constant * (mass * center_gravity) / ((mass/force_multiplier)**2)) / sim_step_speed
            ...
            for other_circle, other_color, other_num, other_mass in circles:
                ...
                if num != other_num:
                    ...
                    velocities[num].x += ((x_minor / (dist_minor ** 2)) * (gravity_constant) * (mass * other_mass) / ((mass/force_multiplier)**2)) / sim_step_speed
                    velocities[num].y += ((y_minor / (dist_minor ** 2)) * (gravity_constant) * (mass * other_mass) / ((mass/force_multiplier)**2)) / sim_step_speed
