...
def main():
    ...
    prev_y = -1
    prev_x = -1
    win = GraphWin('Repulse Attract', x_dims, y_dims) #4/13, dev started 4/14 10am
    ...
    # number of planets
    circle_num = 26 # bring down to 20 if lines enabled
    ...
    space_range.setFill(color_rgb(35,35,35))
    ...
    for (x_offset, y_offset, index) in offsets:
        ...
        #rand_det = randrange(0,2)
        rand_det = index % 2
        charge = (rand_det - 0.5) * 2
        ...
        choose_color = color_rgb((0 if (charge == 1) else 225), (0 if (charge == -1) else 225), 0)
        circle.setFill(choose_color)
        circle.draw(win)
        circles.append((circle, choose_color, index, charge * (planet_mass**2)))
    ...
    while(True):
        ...
        for circle, choose_color, num, mass in circles:
            ...
            velocities[num].x += (x_dist / (curr_dis ** 2)) * gravity_constant * (abs(mass) * center_gravity) / ((mass/force_multiplier)**2)
            velocities[num].y += (y_dist / (curr_dis ** 2)) * gravity_constant * (abs(mass) * center_gravity) / ((mass/force_multiplier)**2)
