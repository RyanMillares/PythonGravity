...
def abs_sqrt(value1):
    return math.sqrt(abs(value1)) * value1/abs(value1)
...
def main():
    ...
    prev_y = -1
    prev_x = -1
    win = GraphWin('Space v11', x_dims, y_dims) #4/19
    ...
    while(True):
        ...
        for circle, choose_color, num, mass in circles:
            collider_velocity_x = 0
            collider_velocity_y = 0
            ...
            velocities[num].x += (x_dist / (curr_dis ** 2)) * gravity_constant * (mass * center_gravity) / ((mass/force_multiplier)**2) * (1 if (curr_dis > (center_radius + circle.getRadius())) else -1)
            velocities[num].y += (y_dist / (curr_dis ** 2)) * gravity_constant * (mass * center_gravity) / ((mass/force_multiplier)**2) * (1 if (curr_dis > (center_radius + circle.getRadius())) else -1)
            center_applied_velocity_x = velocities[num].x
            center_applied_velocity_y = velocities[num].y
            ...
            for other_circle, other_color, other_num, other_mass in circles:
                ...
                if num != other_num:
                    ...
                    if (dist_minor > 1.5*(circle.getRadius() + other_circle.getRadius())):
                        velocities[num].x += (x_minor / (dist_minor ** 2)) * (gravity_constant) * (mass * other_mass) / ((mass/force_multiplier)**2) * ((1 if (dist_minor > 1.5*(other_circle.getRadius() + circle.getRadius())) else -1) if (collision_handler == 1 or (collision_handler == 2 and is_colliding[num])) else 1)
                        velocities[num].y += (y_minor / (dist_minor ** 2)) * (gravity_constant) * (mass * other_mass) / ((mass/force_multiplier)**2) * ((1 if (dist_minor > 1.5*(other_circle.getRadius() + circle.getRadius())) else -1) if (collision_handler == 1 or (collision_handler == 2 and is_colliding[num])) else 1)
                    ...
                    # v2 collision handling
                        ...
                        # 1st version of initial collision handling
                        #collision_velocity_x = collision_velocities[num].x + 2*(other_mass / (mass + other_mass))*(collision_velocities[other_num].x - collision_velocities[num].x)
                        #collision_velocity_y = collision_velocities[num].y + 2*(other_mass / (mass + other_mass))*(collision_velocities[other_num].y - collision_velocities[num].y)

                        # 2nd version of initial collision handling
                        coll_vel_1 = ((2*other_mass)/(mass + other_mass))
                        coll_vel_2 = ((other_mass - mass) / (other_mass+mass))
                        collision_velocity_x = ((coll_vel_1 * collision_velocities[num].x) + (coll_vel_2 * collision_velocities[other_num].x))
                        collision_velocity_x += (collision_velocities[other_num].x - collision_velocities[num].x)
                        collider_velocity_x += collider_velocity_x

                        collision_velocity_y = ((coll_vel_1 * collision_velocities[num].y) + (coll_vel_2 * collision_velocities[other_num].y))
                        collision_velocity_y += (collision_velocities[other_num].y - collision_velocities[num].y)
                        collider_velocity_x += collision_velocity_y
                        ...
                        #velocities[num].x += abs_sqrt(collision_velocity_x)
                        #velocities[num].y += abs_sqrt(collision_velocity_y)
                        #print("resulting velocity: ", velocities[num], "\n")
                    

            if (collision_handler == 2):
                #print("Old: ",velocities[num].x, ", ", velocities[num].y)
                if (colls > 0):
                    if (not is_colliding[num]):
                        velocities[num] = Velocity(collision_velocity_x, collision_velocity_y)
                        is_colliding[num] = True
                        print("Mass ", num, ", ", colls, " colls\n")
                else:
                    is_colliding[num] = False
        ...
        # applies velocity towards current center of mass
        #new_pos = Point(newX, newY)
